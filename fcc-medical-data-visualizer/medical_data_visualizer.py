...
df['overweight'] = 0
df.loc[(df.weight / pow(df.height / 100, 2)) > 25, 'overweight'] = 1
...
def draw_heat_map():
    ...
    sns.heatmap(corr, mask=mask, cbar_kws={"shrink": 0.5}, center=0.0, annot=True, fmt=".1f", linewidths=.5, vmax=0.32, vmin=-0.16)
    # Do not modify the next two lines
